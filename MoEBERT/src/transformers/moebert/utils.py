...
def process_ffn(model):
    """
    Process FFN for both BERT and BART models
    """
    inner_model = None
    ...
    # Check if it's a BART model
    elif hasattr(model, "bart"):
        inner_model = model.bart
    # Check if it's already the inner model
    elif hasattr(model, "encoder") and hasattr(model, "decoder"):
        inner_model = model
    else:
        # Nếu không match thì giữ None
        pass  
    
    # Bạn có thể return None nếu không tìm thấy inner_model
    return inner_model
